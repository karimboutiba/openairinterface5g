...
void update_ptrs_config(NR_CellGroupConfig_t *secondaryCellGroup, int *rbSize, uint8_t *mcsIndex,int8_t *ptrs_arg);
...
int main(int argc, char **argv)
{
  ...
  if (alloc_type == PDSCH_TYPE0) {
    g_rbSize = 0;
    for (int r = 0; r < g_nb_rb_ranges; r++)
      g_rbSize += g_rb_ranges[r].end - g_rb_ranges[r].start + 1;
  }
  ...
  if(enable_ptrs) {
    update_ptrs_config(secondaryCellGroup, &g_rbSize, &mcsIndex, ptrs_arg);
  }
